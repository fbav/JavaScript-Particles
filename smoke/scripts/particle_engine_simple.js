...
function init()
{
  ...
  imgSmoke.src = 'images/newCloud.png';
  imgSmoke.onload = function()
  {
	for (var i = 0; i < numEmiiters; i++)
	{
		emitterArray.push(new ParticleEmitter());
		emitterArray[i].init(0.5*i, -0.4, 10, imgSmoke);
		emitterArray[i].m_alpha = 0.5;
	}
...
{  
  ...
  dirtyRight = 0;
  dirtyBottom = 0;
	for (var i = 0; i < emitterArray.length; i++)
	{
		emitterArray[i].update(timeElapsed);
		emitterArray[i].render(context);
	}
